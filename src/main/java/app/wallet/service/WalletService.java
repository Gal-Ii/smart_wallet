...
public class WalletService {
    ...
    private static final String TOP_UP_DESCRIPTION_FORMAT = "Top-up for %.2f.";
    ...
    @Transactional
    public Transaction withdrawal(User user, UUID walletId, BigDecimal amount, String chargeDescription){
    ...
    private Wallet getById(UUID walletId) {
        return walletRepository.findById(walletId).orElseThrow(() -> new RuntimeException("Wallet by id [%s] does not exist.".formatted(walletId)));
    }

    //Transfer
    // 2 transactions
    // Gosho <> Pesho (20 EUR)
    // Gosho -20 EUR -> 1 withdrawal transaction
    // Pesho +20 EUR -> 1 deposit transaction
    @Transactional
